...
void lrcu_list_check_loop(lrcu_list_head_t *lh){
...
lrcu_list_t *lrcu_list_get_tail(lrcu_list_head_t *lh){
...
#define __lrcu_list_for_each_get___n(n, n_prev, lh) ({ \
            lrcu_list_t *____n = NULL; \
            rmb(); \
            if(n == (lh)->head){ \
                /* to determine "first" iteration */ \
                LRCU_ASSERT(!n_prev); \
                if(n_prev){ \
                    n_prev = (lh)->head; \
                    if(n_prev) \
